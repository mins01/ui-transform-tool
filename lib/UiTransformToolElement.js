class UiTransformToolElement extends HTMLElement{
    ...
    static get observedAttributes() {
        return ['width', 'height','minwidth','minheight','maxwidth','maxheight', 'left', 'top', 'moveable','resizable' ,'disabled' , 'class', 'hide-handle', 'hide-box', 'numerictype'];
    }
    ...
    *{
    ...
    static contentHtml = `
        <div class="container">
            <div class="wrap">
                <slot part="default-slot" class="default-slot"></slot>
                <div part="bounding-box box" class="bounding-box box"><slot name="bounding-box-slot" part="bounding-box-slot"  class="bounding-box-slot"></slot></div>
    ...
    parseNumeric(v){
        return this.integeronly?parseInt(v):parseFloat(v); 
    }
    ...
    get integeronly(){ return this.hasAttribute('integeronly')?true:false }
    set integeronly(v){ if(v){ this.setAttribute('integeronly','') }else{ this.removeAttribute('integeronly') } }

    
    get width(){ return this.parseNumeric(this.getAttribute('width')??100); }
    set width(v){
        ...
        this.setAttribute('width',this.parseNumeric(v));
    }
    get height(){ return this.parseNumeric(this.getAttribute('height')??100); }
    set height(v){
        v = this.parseNumeric(v);
        ...
        this.setAttribute('height',this.parseNumeric(v));
    }
    get minwidth(){ if(!this.hasAttribute('minwidth')){return null;} let minwidth = this.parseNumeric(this.getAttribute('minwidth')); return Number.isFinite(minwidth)?minwidth:null; }
    set minwidth(v){ this.setAttribute('minwidth',v)}
    get minheight(){ if(!this.hasAttribute('minheight')){return null;} let minheight = this.parseNumeric(this.getAttribute('minheight')); return Number.isFinite(minheight)?minheight:null; }
    set minheight(v){ this.setAttribute('minheight',v)}
    get maxwidth(){ if(!this.hasAttribute('maxwidth')){return null;} let maxwidth = this.parseNumeric(this.getAttribute('maxwidth')); return Number.isFinite(maxwidth)?maxwidth:null; }
    set maxwidth(v){ this.setAttribute('maxwidth',v)}
    get maxheight(){ if(!this.hasAttribute('maxheight')){return null;} let maxheight = this.parseNumeric(this.getAttribute('maxheight')); return Number.isFinite(maxheight)?maxheight:null ;}
    set maxheight(v){ this.setAttribute('maxheight',v)}
    get left(){ return this.parseNumeric(this.getAttribute('left')??0); }
    set left(v){ this.setAttribute('left',this.parseNumeric(v))}
    get top(){ return this.parseNumeric(this.getAttribute('top')??0); }
    set top(v){ this.setAttribute('top',this.parseNumeric(v))}
    ...
    static getLayoutInfo(element){
        ...
        const minWidth = element.parseNumeric(cs.minWidth);
        const minHeight = element.parseNumeric(cs.minHeight);
        const maxWidth = element.parseNumeric(cs.maxWidth);
        const maxHeight = element.parseNumeric(cs.maxHeight);

        return {
            width:element.parseNumeric(cs.width),
            height:element.parseNumeric(cs.height),
            left:element.parseNumeric(cs.left),
            top:element.parseNumeric(cs.top),
    ...
    #ondown(event){
        ...
        // event.preventDefault(); // 우선 동작 막지 않음
    ...
    #ondownForMove(event){
        // this.running = 'move'
        this.dispatchEvent((new CustomEvent("utt-move-start",{bubbles:true,composed:true,detail:this.detail})));
    ...
    #onupForMove(event){
        this.dispatchEvent((new CustomEvent("utt-move-end",{bubbles:true,composed:true,detail:this.detail})));
    }

    #ondownForResize(event){
        this.dispatchEvent((new CustomEvent("utt-resize-start",{bubbles:true,composed:true,detail:this.detail})));
    ...
    #onupForResize(event){
        this.dispatchEvent((new CustomEvent("utt-resize-end",{bubbles:true,composed:true,detail:this.detail})));
